(function(window, document, app) {
    ...
    function initQuizPro() {
        // REQ: Estrategia Caché Primero (v7.6)
        if (app) {
            app.init('quizpro', false).then(function() {
    ...
    function navigateToLevels(subjectName, gradeLabel) {
        ...
        if (canUnlockInter) {
            ...
            btnInter.classList.remove('bg-gray-300', 'cursor-not-allowed');
        ...
        if (canUnlockAvan) {
            ...
            btnAvan.classList.remove('bg-gray-300', 'cursor-not-allowed');
    ...
    function backToLevelsAfterResult() {
        document.getElementById('result-screen').classList.add('hidden');
        if (selectedAsignatura) {
            navigateToLevels(selectedAsignatura, selectedGrado);
        } else {
            navigateToSubjects();
        }
    }
    ...
    function calculateXP(isCorrect, level, responseTime) {
        if (app && app.calculateXP) {
            var baseXP = app.calculateXP(isCorrect, level, responseTime, 'quizpro');
                            ...
                            if (selectedCards[0].item.k === selectedCards[1].item.k) {
                                setTimeout(function() {
                                    ...
                                    if (matchedCount === q.pairs.length) {
                                        ...
                                        finishBtn.classList.add('bg-emerald-600', 'animate-bounce', 'shadow-emerald-200');
    ...
    function checkAnswer(selected, correct, btn) {
        ...
        var cleanCorrect = String(finalCorrect || "").trim();
        var isCorrect = false;
    ...
    function endQuiz() {
        ...
        if (typeof fetchApi === 'function') {
            ...
            if (app && app.pendingAnalytics) {
                analyticsPromise = Promise.all(app.pendingAnalytics);
            }
    ...
    function loadGlobalTop() {
        ...
        if (app && app.getLeaderboard) {
            app.getLeaderboard('quizpro', function(data) {
    ...
    function loadPerformanceTable() {
        ...
        if (app && app.getPersonalRecord) {
            app.getPersonalRecord(function(data) {
    ...
    function renderDiagnosis(history) {
        ...
        topicStats = topicStats.filter(function(t) { return t.total >= 5; });
    ...
    function renderPsychometricCharts(history) {
        ...
        var scores = recentLogs.map(function(h) { return parseFloat(h[5]); });
    ...
    window.initQuizPro = initQuizPro;
    window.navigateToSubjects = navigateToSubjects;
    window.navigateToLevels = navigateToLevels;
    window.backToHome = backToHome;
    window.backToSubjects = backToSubjects;
    window.backToLevelsAfterResult = backToLevelsAfterResult;
    window.selectLevel = selectLevel;
    window.loadPerformanceTable = loadPerformanceTable;
    window.submitFib = submitFib;
